    ...
    <div class="grid grid-cols-1 gap-4 mb-4">
      <div>
        ...
        >
          ...
          >
            {{ doctor_table_sections[0].name_th }}
          </h5>
          <p class="font-normal text-gray-700 dark:text-gray-400">
            {{ doctor_table_sections[0].name_en }}
          </p>
        </a>
      </div>
      ...
      <div>
        <div class="mb-4 rounded bg-gray-50 dark:bg-gray-800">
          <div class="relative p-3 overflow-x-auto shadow-md sm:rounded-lg">
            ...
            >
              ...
              >
                <tr>
                  ...
                  <th scope="col" class="px-6 py-3">หัวข้อย่อย (ไทย)</th>
                  <th scope="col" class="px-6 py-3">หัวข้อย่อย (อังกฤษ)</th>
                  <th scope="col" class="px-6 py-3">ไฟล์</th>
              ...
              <tbody>
                <tr
                  v-for="(doctor_table_detail, index) in doctor_table_details.data"
                  :key="doctor_table_detail.id"
                  class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                >
                  <td class="w-4 p-4">
                    <div class="flex items-center">
                      <p>{{ index + doctor_table_details.from }}</p>
                    </div>
                  </td>
                  <th
                    scope="row"
                  ...
                  >
                    <div class="">
                      <p class="overflow-hidden text-ellipsis">{{ doctor_table_detail.name_th }}</p>
                    </div>
                  </th>
                  <td class="px-6 py-4">{{ doctor_table_detail.name_en }}</td>
                  <td class="px-6 py-4">
                    <a :href="doctor_table_detail.files" target="_blank">
                      <svg
                  ...
                  <td class="px-6 py-4">{{ doctor_table_detail.create_date }}</td>
                  <td class="px-6 py-4">
                    <button
                      @click="
                        CopyToModal(doctor_table_detail.id, doctor_table_detail.name_th, doctor_table_detail.name_en)
                      "
                    ...
                    <button
                      @click="
                        deleteSection(doctor_table_detail.id)
                      "
                      type="button"
            ...
            <Pagination
              :links="doctor_table_details.links"
              :datastart="doctor_table_details.from"
              :datacount="doctor_table_details.data.length"
              :datatotal="doctor_table_details.total"
            />
          </div>
        </div>
      </div>
        ...
        <form novalidate @submit.prevent="submit">
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 space-y-4 md:p-5">
                ...
                >หัวข้อย้อย (ไทย)</label
              >
              <textarea
                v-model="formSubSection.name_th"
                id="message"
                rows="4"
              ...
              <label
                for="message"
                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                >หัวข้อย้อย (อังกฤษ)</label
              >
              <textarea
                v-model="formSubSection.name_en"
                id="message"
                rows="4"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Write your thoughts here..."
...
const props = defineProps({
    doctor_table_details: Object,
  doctor_table_sections: Object,
});
...
const formSubSection = useForm({
  ...
  id_manage: "1",
  files: null,
  id_title: props.doctor_table_sections[0].id,
});
...
const CopyToModal = (id, name, year) => {
  formSubSection.id = id;
  formSubSection.name_th = name;
  formSubSection.name_en = year;
  formSubSection.id_manage = "1";
};

const submit = () => {
  formSubSection.post(route("Manage.MDoctorTable_sub_edit"), {});
...
const deleteSection = (id) => {
  dataDelete.id = id;
  dataDelete.post(route("Manage.MDoctorTable_sub_delete"), {});
