...
const form = useForm({
  ...
  News_title: props.Newsdb[0].News_title,
  News_type: props.Newsdb[0].News_type,
  News_title_image: null,
  News_detail: props.Newsdb[0].News_detail,
  News_images: [],
});

const defImage = useForm({
  Timag: props.Newsdb[0].News_title_image,
  Dimag: props.Newsdb[0].News_images,
});
...
const onFileMultipleChange = (e) => {
  const imgmulti = form.News_images;
...
const submit = () => {
  if (form.News_type == "ข่าวจัดซื้อจัดจ้าง") {
    swal({
      title: "สำเร็จ",
    ...
    form.post(route("Manage.MNews_edit_update"), {});
  } else {
    if (
      form.News_title != "" &&
      form.News_title_image != "" &&
      form.News_images != null &&
      form.News_detail != ""
    ) {
      swal({
        title: "สำเร็จ",
...
<template>
  <AdminLayout>
    <h1 class="text-2xl font-bold">แก้ไขข่าว</h1>

    <p class="mt-1 text-gray-600">ข่าว / แก้ไขข่าว</p>
    <div class="p-6 mt-4 bg-white">
      <form novalidate @submit.prevent="submit">
        ...
        >
          ...
          <option>ข่าวจัดซื้อจัดจ้าง</option>
          <option>ข่าวรับสมัครงาน</option>
          <option>ข่าวบริจาก</option>
        </select>
        <label
          for="message"
          class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
        ...
        ></textarea>
        <div v-if="form.News_type !== 'ข่าวจัดซื้อจัดจ้าง'">
          <label
        ...
        <div v-if="form.News_type !== 'ข่าวจัดซื้อจัดจ้าง'">
          ...
          >
          <input
            @input="form.News_images = $event.target.files"
