...
<template>
  <AdminLayout>
    <div class="grid grid-cols-5 gap-4 mb-4">
      <MNews_Anyway :number="NewsType[0]" :title="'ข่าวประชาสัมพัธ์'"></MNews_Anyway>
      <MNews_Anyway
        :number="NewsType[1]"
        :title="'ข่าวสารจัดซื้อ/จัดจ้าง'"
      ></MNews_Anyway>
      <MNews_Anyway :number="NewsType[2]" :title="'ข่าวรับสมัครงาน'"></MNews_Anyway>
      <MNews_Anyway :number="NewsType[3]" :title="'ข่าวบริจาก'"></MNews_Anyway>
      <MNews_Anyway
        :number="'ยังไม่มีในระบบ'"
        :title="'วารสารโรงพยาบาลสตูล'"
      ></MNews_Anyway>
    </div>
    <div class="mb-4 rounded bg-gray-50 dark:bg-gray-800">
      <div class="relative p-3 overflow-x-auto shadow-md sm:rounded-lg">
        ...
        >
          ...
          <tbody>
            <tr
              v-for="(Newss, index) in Newsdb.data"
              :key="Newss.id"
              class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
            >
              <td class="w-4 p-4">
                <div class="flex items-center">
                  <p>{{ index + Newsdb.from }}</p>
                </div>
              </td>
              <th
                scope="row"
              ...
              >
                <div class="">
                  <p class="overflow-hidden text-ellipsis">{{ Newss.News_title }}</p>
                </div>
              </th>
              <td class="px-6 py-4">{{ Newss.News_type }}</td>
              <td class="px-6 py-4">{{ Newss.created_at }}</td>
              <td class="px-6 py-4">
                <Link
                  :href="route('Manage.MNews_edit', Newss.id)"
                  type="button"
                ...
                <Link
                  :href="route('Manage.MNews_move_bin', Newss.id)"
                  type="button"
        ...
        <Pagination
          :links="Newsdb.links"
          :datastart="Newsdb.from"
          :datacount="Newsdb.data.length"
          :datatotal="Newsdb.total"
        />
      </div>
    </div>
    <!-- ---------- add ------------- -->
    ...
    <div data-dial-init class="fixed bottom-6 end-6 group">
      <Link
        :href="route('Manage.MNews_create')"
        type="button"
